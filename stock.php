...
if ($stmt->execute([$product_name, $supplier_name, $quantity, $price, $expiry_date])) {
    // On success, use JavaScript alert
    echo "<script>alert('Stock added successfully!');</script>";
} else {
    // Fetch the error info
    $errorInfo = $stmt->errorInfo();
    // On error, use JavaScript alert
    echo "<script>alert('Error adding stock: " . htmlspecialchars($errorInfo[2]) . "');</script>";
}
}
...
<body>
    ...
    <div class="table-container">
        ...
        <div class="search-container">
            <input type="text" id="stockSearch" onkeyup="searchStock()" placeholder="Search for stock...">
